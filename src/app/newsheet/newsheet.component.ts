...
@Component({
  selector: 'app-newsheet',
  templateUrl: './newsheet.component.html',
  styleUrls: ['./newsheet.component.css'],
})
export class NewsheetComponent implements OnInit {
      ...
      .subscribe(
        (res) => {
          ...
          tempFile.forEach((item) => {
            ...
            if (
              itempath[itempath.length - 1] == 'csv' ||
              itempath[itempath.length - 1] == 'xls' ||
              itempath[itempath.length - 1] == 'xlsx'
            ) {
              temp.push(item);
            }
          });
  ...
  onUSDchange(event) {
    const username = localStorage.getItem('username');
    this.httpClient
      .get<any>(`${environment.api_url}/sheet/newsheetdata/${username}`, {
        headers: new HttpHeaders({ Authorization: 'Bearer ' + this.userToken }),
      })
      .subscribe(
        (res) => {
  ...
  update_remark(item) {
    ...
    // console.log(body);
    this.httpClient
      .post<any>(`${environment.api_url}/sheet/newchangeRemark`, body, {
        headers: new HttpHeaders({ Authorization: 'Bearer ' + this.userToken }),
      })
      .subscribe(
        (res) => {
  ...
  deleteData() {
    ...
    if (
      ...
      this.httpClient
        .delete<any>(
          `${environment.api_url}/sheet/newdeleteData/${username}/${this.selectedCampaign}/${this.selectedDate}`,
          {
  ...
  getRecentData() {
    ...
    this.httpClient
      .get<any>(
        `${environment.api_url}/sheet/newgetdateby/${username}/${searchCampaignTemp}`,
        {
          headers: new HttpHeaders({
            Authorization: 'Bearer ' + this.userToken,
          }),
              ...
              (res) => {
                ...
                if (
                  ...
                  if (
                    ...
                    this.httpClient
                      .get<any>(
                        `${environment.api_url}/sheet/newtempToMain/${username}`,
                        {
                          headers: new HttpHeaders({
                            Authorization: 'Bearer ' + this.userToken,
                          }),
